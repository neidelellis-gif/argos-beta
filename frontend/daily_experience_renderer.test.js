...
test("uses the current fallback cards when an owner has no mapped impact", () => {
    const { elements, renderer } = setup();
    renderer.render(response({ facts: [], priorities: [], analyses: [], impact_assessments: [] }));

    assert.deepEqual(
        elements.get("neiInvestmentImpact").children.map((card) => card.children[0].children[0].textContent),
        ["ETH", "PENDLE", "BTC"]
    );
    assert.deepEqual(
        elements.get("jolikaInvestmentImpact").children.map((card) => card.children[0].children[0].textContent),
        ["AIQ", "EQIX", "GLD"]
    );
});
