...
DEFAULT_PORTFOLIO_SNAPSHOT_DIRECTORY = (
    Path(__file__).with_name("data") / "portfolio_snapshots"
)
DEFAULT_SNAPSHOT_DIRECTORY = DEFAULT_PORTFOLIO_SNAPSHOT_DIRECTORY
_SNAPSHOT_FILENAME_RE = re.compile(
    r"^jolika_\d{8}T\d{6}Z_[0-9a-f]{16}\.json$"
)
_SNAPSHOT_FIELDS = {
    "schema_version",
    "snapshot_id",
...
def _safe_source_file(value: str) -> str:
    ...
    if basename in {"", ".", ".."}:
        raise ValueError("source_file must be a basename")
    return basename
...
def _position_payload(position: PortfolioPosition) -> dict:
    return {
        ...
        "reference_date": (
            None
            if position.reference_date is None
            else position.reference_date.isoformat()
        ),
        "source_file": _safe_source_file(position.source_file),
        "economic_asset_class": (
            None
            if position.economic_asset_class is None
            else position.economic_asset_class.value
        ),
    }
...
def _ordered_positions(
    ...
    for position in positions:
        ...
        if instance.instance_key in indexed:
            raise ValueError(
                f"Duplicate JOLIKA snapshot instance_key: {instance.instance_key}"
            )
        _position_payload(position)
        indexed[instance.instance_key] = position
    if not indexed:
...
def build_portfolio_snapshot(
    ...
    *,
    ...
    captured_at_utc = _require_aware(captured_at).replace(microsecond=0)
    ordered = _ordered_positions(tuple(positions))
    institutions = tuple(
        sorted({position_snapshot_identity(position).institution for position in ordered})
    )
    source_files = tuple(
        sorted({_safe_source_file(position.source_file) for position in ordered})
    )
...
def _validated_snapshot(snapshot: PortfolioSnapshot) -> PortfolioSnapshot:
    ...
    rebuilt = build_portfolio_snapshot(
        snapshot.positions,
...
def _require_exact_fields(record: dict, expected: set[str], artifact: str) -> None:
    ...
    if unexpected:
        raise ValueError(
            f"Unexpected {artifact} fields: {', '.join(sorted(unexpected))}"
        )
...
def _parse_position(record: dict) -> PortfolioPosition:
    ...
    try:
        economic_class = (
            None if raw_economic is None else EconomicAssetClass(raw_economic)
        )
    except (TypeError, ValueError) as exc:
    ...
    return PortfolioPosition(
        ...
        asset_subclass=_parse_optional_string(
            record["asset_subclass"], "asset_subclass"
        ),
        ...
        identifier_type=_parse_optional_string(
        ...
        portfolio_weight=_parse_decimal(
            record["portfolio_weight"], "portfolio_weight"
        ),
...
def select_previous_portfolio_snapshot(
    ...
    *,
    ...
    eligible = [
        snapshot for snapshot in materialized if snapshot.captured_at < cutoff
    ]
    if not eligible:
        return None
