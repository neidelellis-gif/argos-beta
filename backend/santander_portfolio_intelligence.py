"""Factual, read-only intelligence for the JOLIKA portfolio held at Santander."""
...
@dataclass(frozen=True)
class SantanderPortfolioIntelligence:
    """Immutable structural intelligence for Santander/JOLIKA positions."""
...
def build_santander_portfolio_intelligence(
    positions: Iterable[PortfolioPosition],
    *,
    top_n: int = 10,
) -> SantanderPortfolioIntelligence:
    """Build deterministic structural intelligence for Santander/JOLIKA positions."""
    ...
    for position in canonical_positions:
        if position.institution != "Santander":
            raise ValueError("all positions must belong to Santander")
    ...
    return SantanderPortfolioIntelligence(
