...
def analyze_request(data: Dict) -> Dict:
    from backend.connectors.ubs_connector import load_positions as load_ubs_positions
    from backend.connectors.santander_connector import load_positions as load_santander_positions
    from backend.consolidation import consolidate_positions

    portfolio = data.get("portfolio")
    if portfolio != "JOLIKA":
        raise ValueError("Somente a análise JOLIKA está disponível nesta release.")
    ...
    if not ubs_payload or not santander_payload:
        raise ValueError("Os arquivos UBS e Santander são necessários.")

    ubs_name = ubs_payload.get("name", "ubs.csv")
    santander_name = santander_payload.get("name", "santander.xlsx")

    ubs_bytes = _decode_file_payload(ubs_payload)
    santander_bytes = _decode_file_payload(santander_payload)

    ubs_path = _save_temp_file(ubs_bytes, suffix=Path(ubs_name).suffix)
    santander_path = _save_temp_file(santander_bytes, suffix=Path(santander_name).suffix)

    try:
        ubs_positions = load_ubs_positions(str(ubs_path))
        santander_positions = load_santander_positions(str(santander_path))
        result = consolidate_positions(ubs_positions, santander_positions)
        return {"ok": True, "result": result}
    finally:
        for path in (ubs_path, santander_path):
            try:
                path.unlink()
            except OSError:
                pass
...
class ArgosRequestHandler(http.server.SimpleHTTPRequestHandler):
...
if __name__ == "__main__":
    ...
    with socketserver.TCPServer(("localhost", PORT), ArgosRequestHandler) as httpd:
        ...
        print("Serving frontend and /api/analyze")
        httpd.serve_forever()
