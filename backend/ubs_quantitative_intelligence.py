"""Quantitative, read-only market intelligence for the JOLIKA portfolio held at UBS."""
...
class UBSPositionQuantitativeMetrics:
    """Historical market-risk metrics for one UBS/JOLIKA position."""
...
@dataclass(frozen=True)
class UBSQuantitativeAssessment:
    """Aggregated quantitative assessment for the UBS/JOLIKA portfolio."""
...
@dataclass(frozen=True)
class UBSQuantitativeIntelligence:
    """Immutable quantitative intelligence for UBS/JOLIKA positions."""
    ...
    positions: tuple[UBSPositionQuantitativeMetrics, ...]

    assessment: UBSQuantitativeAssessment
    attention_positions: tuple[UBSPositionQuantitativeMetrics, ...]
...
) -> UBSPositionQuantitativeMetrics:
    return UBSPositionQuantitativeMetrics(
...
def _assessment(
    ...
    return UBSQuantitativeAssessment(
        level=level,
        reasons=reasons,
...
def _attention_sort_key(
    item: UBSPositionQuantitativeMetrics,
) -> tuple:
    level_rank = {
        "Alta": 0,
        "Média": 1,
...
def _attention_positions(
    positions: tuple[UBSPositionQuantitativeMetrics, ...],
) -> tuple[UBSPositionQuantitativeMetrics, ...]:
    return tuple(
        sorted(
            (
                item
    ...
    *,
    ...
    if assessment.analyzed_position_count == 0:
        return (
            ...
            "uma leitura quantitativa da carteira UBS."
        )

    if assessment.level == "Alta":
        reading = (
            "A leitura quantitativa da UBS mostra risco histórico elevado "
            "em pelo menos uma posição e isso merece atenção prioritária."
        )
    elif assessment.level == "Média":
        reading = (
            "A leitura quantitativa da UBS mostra alguns pontos de risco "
            "histórico acima do nível mais confortável e que merecem acompanhamento."
        )
    else:
        reading = (
            "A leitura quantitativa da UBS não mostra, neste momento, "
...
def build_ubs_quantitative_intelligence(
    positions: Iterable[PortfolioPosition],
    *,
    market_connector: MarketConnector,
    lookback_days: int = 365,
) -> UBSQuantitativeIntelligence:
    """Build historical market-risk intelligence for UBS/JOLIKA positions."""
    ...
    for position in canonical_positions:
        if position.institution != "UBS":
            raise ValueError(
                "all positions must belong to UBS"
            )
    ...
    return UBSQuantitativeIntelligence(
        institution="UBS",
