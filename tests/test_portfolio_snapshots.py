...
def test_captured_at_is_canonicalized_to_seconds():
    precise = datetime(
        2026, 8, 12, 19, 41, 1, 123456, tzinfo=timezone.utc
    )
    snapshot = build_portfolio_snapshot((pos(),), captured_at=precise)

    assert snapshot.captured_at == datetime(
        2026, 8, 12, 19, 41, 1, tzinfo=timezone.utc
    )
    assert snapshot.captured_at.microsecond == 0


def test_microsecond_captured_at_round_trip_is_exact(tmp_path):
    precise = datetime(
        2026, 8, 12, 19, 41, 1, 123456, tzinfo=timezone.utc
    )
    snapshot = build_portfolio_snapshot((pos(),), captured_at=precise)
...
def test_same_second_different_microseconds_have_same_snapshot_id():
    ...
    second = build_portfolio_snapshot(
        (pos(),),
        captured_at=datetime(
            2026, 8, 12, 19, 41, 1, 987654, tzinfo=timezone.utc
        ),
    )
...
def test_different_seconds_remain_distinct_snapshots():
    first = build_portfolio_snapshot(
        (pos(),),
        captured_at=datetime(
            2026, 8, 12, 19, 41, 1, 123456, tzinfo=timezone.utc
    ...
    second = build_portfolio_snapshot(
        (pos(),),
        captured_at=datetime(
            2026, 8, 12, 19, 41, 2, 123456, tzinfo=timezone.utc
        ),
    )
