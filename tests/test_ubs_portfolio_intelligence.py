...
from backend.ubs_portfolio_intelligence import build_ubs_portfolio_intelligence
...
def test_empty_input_returns_empty_ubs_jolika_intelligence():
    result = build_ubs_portfolio_intelligence([])
...
def test_result_and_nested_models_are_frozen():
    result = build_ubs_portfolio_intelligence([position()])
...
def test_valid_ubs_jolika_position_is_accepted():
    result = build_ubs_portfolio_intelligence([position()])
...
@pytest.mark.parametrize(
    ("positions", "message"),
    [
        ([position(institution="Santander")], "UBS"),
        ([position(owner=PortfolioOwner.NEI)], "JOLIKA"),
        ([position(), position(institution="Santander")], "UBS"),
        ([position(), position(owner=PortfolioOwner.NEI)], "JOLIKA"),
    ],
)
def test_rejects_non_ubs_or_non_jolika_input(positions, message):
    with pytest.raises(ValueError, match=message):
        build_ubs_portfolio_intelligence(positions)


@pytest.mark.parametrize("top_n", [0, -1, 1.5, "3", True, False, None])
def test_rejects_invalid_top_n(top_n):
    with pytest.raises(ValueError, match="positive integer"):
        build_ubs_portfolio_intelligence([], top_n=top_n)
...
def test_calls_official_diagnostic_exactly_once():
    ...
    with patch(
        "backend.ubs_portfolio_intelligence.diagnose_institution",
...
def test_identifier_none_is_preserved_and_asset_label_falls_back():
    ...
    result = build_ubs_portfolio_intelligence([unnamed])
...
def test_asset_label_prefers_name_then_identifier():
    ...
    result = build_ubs_portfolio_intelligence([named, identifier_only])
...
def test_input_order_does_not_change_aggregate_facts():
    ...
    left = build_ubs_portfolio_intelligence(positions)
    right = build_ubs_portfolio_intelligence(reversed(positions))
...
def test_no_float_is_introduced_anywhere_in_monetary_output():
    result = build_ubs_portfolio_intelligence([position(value="10.25")])
