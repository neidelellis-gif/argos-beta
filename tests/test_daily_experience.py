...
from backend.config.environment import load_environment_file
...
def test_environment_file_is_loaded_without_overriding_process(tmp_path, monkeypatch):
    env_file = tmp_path / ".env"
    env_file.write_text(
        "FINNHUB_API_KEY=from-file\nARGOS_DAILY_PROVIDERS=finnhub\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("FINNHUB_API_KEY", "from-process")
    monkeypatch.delenv("ARGOS_DAILY_PROVIDERS", raising=False)

    assert load_environment_file(env_file) is True
    assert __import__("os").environ["FINNHUB_API_KEY"] == "from-process"
    assert __import__("os").environ["ARGOS_DAILY_PROVIDERS"] == "finnhub"


def test_successful_finnhub_response_populates_important_facts(tmp_path):
    requested = []
    ...
    def opener(url, timeout):
        ...
        return Response((
            '[{"id": 123, "headline": "NVIDIA atualiza plataforma", '
            '"summary": "Comunicado corporativo", "datetime": 1785250800, '
            '"source": "Reuters", "related": "NVDA"}]'
        ).encode("utf-8"))

    provider = FinnhubDailyProvider(api_key="x", opener=opener)
    result = service(tmp_path, provider).generate((), NOW)

    assert result["sources"]["facts"]["status"] == "available"
    assert result["facts"][0]["title"] == "NVIDIA atualiza plataforma"
    parsed_url = urlparse(requested[0][0])
    assert parsed_url.scheme == "https"
    assert parsed_url.netloc == "finnhub.io"
    assert parsed_url.path == "/api/v1/news"
    assert set(parse_qs(parsed_url.query)) == {"category", "minId", "token"}
    assert requested[0][1] == 10


def test_finnhub_error_payload_is_exposed_as_unavailable(tmp_path):
    class Response(BytesIO):
        def __enter__(self):
            return self
    ...
    assert result["sources"]["facts"]["status"] == "unavailable"
    assert "API limit reached" in result["sources"]["facts"]["error"]
